...
##### Fit polynomial regression with orders: 
...
!!! note "Underfitting"
	**Underfitting**: the models are overly simple
"""
...
!!! note "Overfitting"
	When the models are overly complicated models
    * checking training errors only does not help
    * **training loss**  favours complicated models
"""
...
## Other under/overfitting examples

When the **number of basis** is too (_small_) _large_
* **too many new features** lead to (**underfitting**) **overfitting**
"""
...
# ╔═╡ fb52704b-d363-4030-935d-33c90bd7917c
md"""
...
We can also set the gradient to zero, and solve it **analytically**
...
# ╔═╡ e6b7146e-3fca-43d1-9764-3216796d69d1
md"""
...
```math
\begin{align}
	\nabla L(\mathbf{w}) &= \mathbf{X}^\top(\mathbf{Xw}- \mathbf{y} ) + \lambda\mathbf{w} = \mathbf{0} \\
	&\Rightarrow \mathbf{X}^\top\mathbf{Xw}- \mathbf{X}^\top\mathbf{y}  + \lambda\mathbf{w}= \mathbf{0} \tag{expand matrix}
\end{align}
```
...
# ╔═╡ 96512b63-7b93-46e5-a4bf-eb27f85bd8fb
md"""
...
\begin{align}
	\nabla L(\mathbf{w}) &= \mathbf{X}^\top(\mathbf{Xw}- \mathbf{y} ) + \lambda\mathbf{w} = \mathbf{0} \\
	&\Rightarrow \mathbf{X}^\top\mathbf{Xw}- \mathbf{X}^\top\mathbf{y}  + \lambda\mathbf{w}= \mathbf{0} \tag{expand matrix}\\
	&\Rightarrow (\mathbf{X}^\top\mathbf{X} +\lambda\mathbf{I})\mathbf{w} = \mathbf{X}^\top\mathbf{y} \tag{$\lambda \mathbf{w} = \lambda \mathbf{Iw}$}
\end{align}
```
...
\begin{align}
	...
	&\Rightarrow \mathbf{X}^\top\mathbf{Xw}- \mathbf{X}^\top\mathbf{y}  + \lambda\mathbf{w}= \mathbf{0} \tag{expand matrix}\\
	&\Rightarrow (\mathbf{X}^\top\mathbf{X} +\lambda\mathbf{I})\mathbf{w} = \mathbf{X}^\top\mathbf{y} \tag{$\lambda \mathbf{w} = \lambda \mathbf{Iw}$}\\
	&\Rightarrow \mathbf{w} = (\mathbf{X}^\top\mathbf{X} +\lambda\mathbf{I})^{-1}\mathbf{X}^\top\mathbf{y}\tag{multiply inverse}
\end{align}
```
...
We can apply **gradient descent** to estimate ``\mathbf{w}_{ridge}``
...
Sub-in the gradient for ``\nabla L(\mathbf{w}_{old})``:


```math
\large
\mathbf{w}_{new} = \underbrace{\mathbf{w}_{old} - \gamma\cdot\,  \mathbf{X}^\top(\mathbf{Xw}_{old}- \mathbf{y})}_{\text{gradient descent of normal regression}} \; \boxed{\colorbox{lightblue}{$- \gamma\, \lambda\mathbf{w}_{old}$}}_{\text{weight decay}}
```
...
for i in range(0, max_iters):
		error = Xs@w - ys
        grad =  Xs.T@error
        # update with the extra weight decay
        w = w - gamma * grad -  gamma * lambda_ * w
		loss = np.sum(error ** 2) / 2 # original loss
		loss += lambda_ / 2 * np.sum(w ** 2) # L2 penalty
...
let
	...
	loss_test = norm([poly_fun(x, w) for x in x_poly_test] - y_poly_test)/length(y_poly_test)
	plot!(-2:0.01:2, (x) -> poly_fun(x, w), lw=2, lc=:red,  label="", legend=:outerbottom, ylim=[-7, 7], title="test loss: "*L"%$(round(loss_test; digits=2));\;" * L"\lambda=%$(round(λ; digits=1))")
		# push!(plots_, plt)
	# end
	# plot(plots_..., size=(900, 600))
end
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "6973eb740f028c3d214776aae7b9484cdd72b6cb"
