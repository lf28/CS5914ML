...
Gaussian distribution, random variable ``y`` is Gaussian distributed if

$$p(y|\mu, \sigma^2) = \mathcal{N}(y; \mu, \sigma^2) = \frac{1}{ \sqrt{2\pi\sigma^2}} e^{-\frac{1}{2} \frac{(y-\mu)^2}{\sigma^2} }$$
...
$$p(y|\mu,\sigma^2) = \mathcal{N}(y; \mu, \sigma^2) = \frac{1}{\sqrt{2\pi \sigma^2}} \exp \left \{-\frac{1}{2} \frac{(y-\mu)^2}{\sigma^2} \right \}$$
...
Probabilistic regression: a probabilistic generative model
  * we tell a story about how ``y^{(i)}`` being generated



And we assume each $y^{(i)}$ is generated by
...
* __*Signal*__ (non random) + random **Gaussian** **_noise_** (random)
...
let
	...
	if add_yi
		scatter!([xis[i]],[ys[i]], markershape = :circle, label="observation: "*L"y^{(i)}", c=1, markersize=4)
	end
...
In summary, a probabilistic linear regression model:

* the unknown (hypothesis) are: ``\mathbf{w}, \sigma^2``

* the data observations are: ``\mathcal{D} =\{y^{(1)}, y^{(2)}, \ldots, y^{(n)}\}``
  * note that ``\{\mathbf{x}^{(i)}\}`` are assumed fixed and non-random
...
# ╔═╡ 25610313-5ad7-4219-861d-bf12251d2a37
md"""
...
Recall MLE aims at **maximising** ``\ell(\mathbf{w})``, which is the same as minimising its **negation** 
...
&\Rightarrow {\mathbf{w}}_{\text{MLE}} \leftarrow \arg\min_{\mathbf{w}} \left (\underbrace{\cancel{\frac{n}{2} \ln 2\pi\sigma^2}}_{\text{constant}} +\frac{1}{2\cancel{\sigma^2}} \sum_{i=1}^n({y}^{(i)}-\mathbf{w}^\top \mathbf{x}^{(i)})^2\right )\\
&\Rightarrow {\mathbf{w}}_{\text{MLE}} \leftarrow \arg\min_{\mathbf{w}}  \frac{1}{2} \underbrace{\sum_{i=1}^n({y}^{(i)}-\mathbf{w}^\top \mathbf{x}^{(i)})^2}_{\text{sum of squared error loss!}}
\end{align}
```

Therefore, by reusing the normal equation result:
...
## Observation noise scale ``\sigma^2_{\text{MLE}}`` 


Difference: probabilistic linear regression has one additional parameter $\sigma^2$ 
...
```math
\large
{\sigma^2}_{\text{MLE}} = \frac{1}{n} \sum_{i=1}^n (y^{(i)} -\mathbf{w}^\top\mathbf{x}^{(i)})^2
```
"""
...
!!! fact "Summary"
	...
	```math
		...
		\sigma^2_{\text{MLE}} &= \frac{1}{n} \sum_{i=1}^n (y^{(i)} -\mathbf{w}^\top\mathbf{x}^{(i)})^2
		\end{align}
	```

"""

# ╔═╡ 257e4f37-8395-4fc7-b6ad-1564563a3c9d
md"""
...
Recall the optimisation objective is
...
We may want to optimise it with **gradient descent**


> ```math
> \large
> \begin{align}
> \begin{bmatrix}\mathbf{w}_{new}\\ {\sigma^2}_{new} \end{bmatrix} = \begin{bmatrix}\mathbf{w}_{old}\\ {\sigma^2}_{old} \end{bmatrix} - \gamma   \nabla_{\mathbf{w},\sigma^2} \ell(\mathbf{w}_{old}, \sigma^2_{old})
> \end{align}
> ```


* assume ``\ell`` is the negative log likelihood 

"""

# ╔═╡ a3756d13-91e1-463e-827d-0d8b7845a7e6
md"""
...
We may want to optimise with **gradient descent**
...
**However,** the standard deviation ``\large \sigma`` and ``\large \sigma^2`` are strictly positive, *i.e.* 
...
* this is a **constrained** optimisation problem
  * gradient descent might violate the constraint
* *on the other hand*, ``\mathbf{w}`` is **un-constrained**

"""

# ╔═╡ bb3a43e0-647e-4ee3-b70a-2ec530d0bafa
md"""

## Constrained optimisation

To deal with the constraint, we usually optimise ``\large \ln\sigma`` instead

* ``\ln\sigma`` is **un-constrained**
* this is known as *reparameterisation*

"""

# ╔═╡ 84afde94-1aa9-4759-a1ee-8fc0cfc00dec
md"""

**Before reparameterisation**

```math
\large
\begin{align}
 \hat{\mathbf{w}}, \hat{\sigma^2} \leftarrow \arg\min_{\mathbf{w}} \left ({{\frac{n}{2} \ln \sigma^2}} +\frac{1}{2{\sigma^2}} \sum_{i=1}^n({y}^{(i)}-\mathbf{w}^\top \mathbf{x}^{(i)})^2 + C\right )
\end{align}
```
**After reparameterisation** with ``\ln\sigma``

```math
\large
\begin{align}
 \hat{\mathbf{w}}, \colorbox{pink}{$\hat{\ln\sigma}$}\leftarrow \arg\min_{\mathbf{w}} \left ({{n \colorbox{pink}{$\ln\sigma$}}} +\frac{1}{2{e^{(2\colorbox{pink}{$\ln\sigma$})}}} \sum_{i=1}^n({y}^{(i)}-\mathbf{w}^\top \mathbf{x}^{(i)})^2 + C\right )
\end{align}
```
"""

# ╔═╡ e4258f13-9f17-4f86-9af6-98ed92daddd2
md"""
...
> \large
> \begin{align}
> \begin{bmatrix}\mathbf{w}_{new}\\ \colorbox{pink}{$\ln{\sigma}_{new}$} \end{bmatrix} = \begin{bmatrix}\mathbf{w}_{old}\\ \colorbox{pink}{$\ln{\sigma}_{old}$} \end{bmatrix} - \gamma   \nabla_{\mathbf{w},\colorbox{pink}{$\ln{\sigma}$}} \ell(\mathbf{w}_{old}, \colorbox{pink}{$\ln{\sigma}_{old}$})
> \end{align}
> ```
