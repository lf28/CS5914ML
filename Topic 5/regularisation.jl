...
# ╔═╡ cc6bfa8c-3251-47b0-95f6-1801d8d81a89
ChooseDisplayMode()

# ╔═╡ af289cb1-0b60-4c0f-ac74-d384d4a27ab3
md"""
...
## MLE is problematic -- overfitting
...
* the data clusters are close to each other
"""
...
```math
\Large \mathbf{w} \rightarrow \infty\; \Longrightarrow\; \text{loss} \rightarrow 0
```


* MLE believes: the sharper the better! but overfitting 
"""
...
Given ``\large \mathbf{x}_{test} \in \mathbb{R}^m``,

```math
\large
\begin{align}
\hat{y}_{test} = 1; \;\; & \text{if }p(y =1 |\hat{\mathbf{w}}_{\text{ML}}, \mathbf{x}_{test}) =\sigma(\hat{\mathbf{w}}_{\text{ML}}^\top\mathbf{x}_{test}) \geq 0.5 \\

\hat{y}_{test} = 0; \;\; & \text{if }p(y =1 |\hat{\mathbf{w}}_{\text{ML}}, \mathbf{x}_{test}) =\sigma(\hat{\mathbf{w}}_{\text{ML}}^\top\mathbf{x}_{test}) < 0.5
\end{align}
```
"""
...
All points on one side are predicted either with 1 or 0
* points next to the boundary are treated the same as those further away
"""
...
!!! note "Solution: regularisation or MAP"
	Add a penalty term or MAP estimator
	```math
	\large
		\hat{\mathbf{w}}_{\text{MAP}}\leftarrow \arg\min_{\mathbf{w}} \underbrace{\frac{1}{n} \sum_{i=1}^n L^{(i)}(\mathbf{w}) + \boxed{\frac{\lambda }{2} \mathbf{w}^\top \mathbf{w}}_{\rm penalty}}_{\text{regularised loss}}
	```
...
```math
\large
\frac{1}{n} \sum_{i=1}^n\nabla L^{(i)}(\mathbf{w})  + \lambda \cdot \mathbf{w}
```
* weight decay as usual
"""

# ╔═╡ f70b3aa6-efa7-4b99-b08b-235930294009
aside(tip(md"""
...
* ``\textcolor{blue}{\text{MLE}}`` converge to larger scale
* ``\textcolor{orange}{\text{MAP}}``  more regularised; thanks to regularisation (or add the zero mean Gaussian prior)
...
The regularised or MAP prediction
...
but more regularised or actually rational
* no longer black and white
* data points closer to the decision boundary's prediction is less certain than those further away 
"""

# ╔═╡ 32e83950-4f08-48dc-a74f-b6b701a6c180
md"""
...
*Daniel Vigot Godoy* (2021) *Deep Learning with PyTorch Step by Step* Chapter 2
"""

# ╔═╡ 24c9b18b-e443-4473-bf16-a94c04f84b44
# using MLJBase

# ╔═╡ 6a8704d6-3106-4b3d-9f41-33337718472c
# begin
# 	X, y = make_moons(70; noise=0.3, rng = 1)
# 	dfHalfCircles = DataFrame(X)
# 	dfHalfCircles.y = y
# end

# ╔═╡ d8d9314f-81b1-4c33-821c-00b019bebd96
# using DataFrames

# ╔═╡ e0440244-bbd0-4c6f-b37b-5322ac78b4dc
md"""

## Classification threshold
"""

# ╔═╡ 27907341-96b1-45b1-9cc4-07aca51086b2
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/class_threshold.png
' width = '800' /></center>"

# ╔═╡ 769b0763-6446-42c4-b13b-4d43a8a103fd
md"""

## Confusion matrix
"""

# ╔═╡ ddfbab25-1e80-43cf-bf15-79db56dc8f29
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/threshold_error.png
' width = '800' /></center>"

# ╔═╡ a62e4df1-5d1e-4d1f-8cda-c72f4058c614


# ╔═╡ f497e79a-adc6-4995-a872-d40732a24c3e
md"""

## Confusion matrix
"""

# ╔═╡ c5107702-0519-45d0-939c-4d4b6df49eea
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/confu_mat.png
' width = '500' /></center>"

# ╔═╡ 056c86c8-79b6-4ce0-a960-7c1fd7d76863
md"[Source](https://www.v7labs.com/blog/confusion-matrix-guide)"

# ╔═╡ 25d04bca-bf26-4d9c-8672-5688cc78b08c
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/threshold_confu.png
' width = '800' /></center>"

# ╔═╡ 75b025e2-a74d-4e20-9687-546036677d05
md"""
...
# ╔═╡ 3ba2a014-e3f2-4f08-a1b8-d507c86c7e75
md"""
...
# ╔═╡ 0ac9194d-89ae-4b43-981d-145c667881e1
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/threshold_confu.png
' width = '800' /></center>"

# ╔═╡ a768d716-4670-42b4-8108-35df7cb9a6df
md"""

$$\large \text{TPR} = \frac{\text{TP}}{\text{TP + FN}} =  \frac{10}{10+1} \approx 0.91; \ \ \  \text{FPR} = \frac{\text{FP}}{\text{FP + TN}} = \frac{2}{2+7}\approx 0.22$$

"""

# ╔═╡ e91ce6ef-9103-4a29-9b57-6384e3da461f
md"""
$$\large \text{Accuracy} = \frac{\text{TP+TN}}{\text{TP+TN+FP+FN}} = \frac{10+7 }{10+7+2 + 1}= 0.85$$

"""

# ╔═╡ 9ea5e314-f024-41c7-8299-f9601c6a5cae
md"""
...
# ╔═╡ 29fe24c1-6dc4-4bca-a5a5-e39cffc486d3
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/threshold_confu.png
' width = '800' /></center>"

# ╔═╡ 689a46b3-403b-4655-b170-e65cfedec204
md"""

$$\text{Recall} = \frac{\text{TP}}{\text{TP + FN}} =  \frac{10}{10+1} \approx 0.91; \ \ \  \text{Precision} = \frac{\text{TP}}{\text{TP + FP}} = \frac{10}{10+2}\approx 0.83$$

"""

# ╔═╡ 9f71f68f-fe06-459f-b07f-c940de593457
md"""

## Trade-offs and Trade-off curves



"""

# ╔═╡ 50d2aeae-cb2c-49af-b2b8-437e9545632c
TwoColumn(md"""
$$\text{FPR}(\text{thred}=0.5) \approx 0.22$$
$$\text{TPR}(\text{thred}=0.5)  \approx 0.91$$ 

""", md"""$$\text{Recall}(\text{thred}=0.5)  \approx 0.91$$ 
$$\text{Precision}(\text{thred}=0.5) \approx 0.83$$
""")

# ╔═╡ e8681190-8e06-4e06-bb8f-b73c4b800001
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/roc1.png
' width = '800' /></center>"

# ╔═╡ 6d3da35e-b51d-4ea0-88c9-d207d4e8fbb5
md"""

## Low threshold
"""

# ╔═╡ 044a1b2d-3c39-495a-8078-9a8f96d9adf8
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/class_threshold_3.png
' width = '800' /></center>"

# ╔═╡ f25a4918-7156-4ae8-9d23-d66068b9e24a
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/roc2.png
' width = '800' /></center>"

# ╔═╡ 4db8a0f0-99f4-4032-93cc-d214a40a1595
md"""

## High threshold
"""

# ╔═╡ 369741b8-63b8-4c09-af5d-266053b9794e
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/class_threshold7.png
' width = '800' /></center>"

# ╔═╡ 8ad5b346-bc11-42f4-a371-fc27de6f78ef
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/roc3.png
' width = '800' /></center>"

# ╔═╡ 4a1f2830-a6f0-40d1-8307-51eb2cbaca52
md"""

## Connect the dots
"""

# ╔═╡ a085b713-7196-4395-aad9-6f23521aedfa
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/roc4.png
' width = '800' /></center>"

# ╔═╡ 9505cbf9-e9fe-4391-bd9a-95e82789294d
md"""

## The best and worst curves
"""

# ╔═╡ df1c83a4-9521-48b6-86e0-6b4c198bfb5d
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/roc4_.png
' width = '800' /></center>"

# ╔═╡ 0ff1ba3b-3b0e-4332-aaf2-5f67ef732b93
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/roc5.png
' width = '800' /></center>"

# ╔═╡ d1bb2d09-0608-4014-84ba-0ac00d712c0a
md"""
## Put it together

"""

# ╔═╡ 6a00e5c0-303a-46e0-82e6-7dbaa0aa079d
html"<center><img src='https://upload.wikimedia.org/wikipedia/commons/thumb/1/13/Roc_curve.svg/2560px-Roc_curve.svg.png
' width = '500' /></center>"

# ╔═╡ a69ee217-3ad8-4d16-ab3b-abdc6da0fad2
md"[Source](https://en.wikipedia.org/wiki/Receiver_operating_characteristic)"
...
# ╔═╡ 1671cd3e-0759-4f17-b69d-ec22ce7e9ed8
begin
	using GLM
...
# ╔═╡ b17be94c-8e05-47a6-bf76-8dcd57a58088
predict(w, X) = logistic.(X * w) .> 0.5

# ╔═╡ 9a16a833-1626-471a-a603-3801646bee9e
begin
...
let
	...
	p_freq_1 = Plots.contour(minx:0.1:maxx, miny:0.1:maxy, (x, y) -> prediction(freq_coef, x, y), fill=false, connectgaps=true, levels=10, line_smoothing = 0.85, legend=:left, title="MLE Prediction", c=:jet, ratio=1.0, xlabel=L"x_1", ylabel=L"x_2", colorbar=false)
	...
	xs, ys = meshgrid(minx:0.1:maxx, miny:0.1:maxy)
	colors = [prediction(freq_coef, x, y) > 0.5   for (x, y) in zip(xs, ys)] .+ 1
	scatter!(xs, ys, color = colors,
            markersize = 1.5, label = nothing, markerstrokewidth = 0, markeralpha=0.9, xlim= [minx, maxx], ylim =[miny, maxy], framestyle=:semi)
...
# ╔═╡ 08d9ff3d-2c8d-4cff-9433-6207c409ceac
p_freq_1= let
	gr()
	freq_coef = coef(glm_fit)'
	# prediction(ws, x1, x2) = logistic(ws * [1.0, x1, x2])
...
# ╔═╡ 3e206e07-204c-40aa-b543-e2816ed3a6eb
p_freq_2= let
	gr()
...
# ╔═╡ 67f08d4a-8e65-4747-8760-8dfba4f060d9
TwoColumn(plot(p_freq_1, size=(350,300)),plot(p_freq_2, size=(350,300)))

# ╔═╡ 5129ada0-4c3a-4310-bbe4-0ad950b533b2
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "673bec79f38c01e2572f7e0963dc7c09ba120f2d"
