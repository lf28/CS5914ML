...
# ╔═╡ cbbcfa68-b539-4be3-a1f5-85616d243135
using Zygote
...
begin
	next1
	topics = ["Vectors & Matrices", "Vector operations: scaling, addition, inner product", "Matrix operations: linear transformation, inverse, pseudo inverse", "Rotation, Projection"]
...
# ╔═╡ e69aa921-91ec-47ea-a55a-a898f7a78359
begin

	function f(θ; logf = true)  
		x, y = sin(θ), cos(θ)
		lf = log(2 * x + 4 * y +5) - log(x + y +1)
		logf ? lf : exp(lf)
	end
end

# ╔═╡ e8f893dd-d50c-4d09-ba30-f78565a37bc1
plot(-((π/2)):0.001:π, (x) -> f(x; logf=true), xlabel=L"x", ylabel=L"\ln f(x)", label="")

# ╔═╡ 5b9b1dab-304f-40ff-809d-29b9f30af3c0
begin
	x0 = -1.
	γ = 0.1
	fs = []
...
# ╔═╡ 07489405-7252-454e-b6c3-9f295354ceb3
plot(fs)

# ╔═╡ a255a5f3-19cd-409d-8aed-49dc8c3070e0
begin

	plot(-((π/2)):0.001:π, (x) -> f(x; logf=true), ylim=(0, 20))

	sch = cgrad(:thermal, rev = true)
	xis = 1:10:length(xs)
	anim = @animate for (i, xi) in enumerate(xis)
		ci = i / length(xis)
		# println(ci)
		plot!([xs[xi]], [f(xs[xi])], st=:scatter, c = get(sch, ci), label="", title="Itertion: "*L"%$(xi)")
	end 
end

# ╔═╡ 19857d9a-63bb-447e-996c-a8a6b4c535c2
gif(anim)
