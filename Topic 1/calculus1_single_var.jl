...
```math
\Large
\lVert\mathbf{d} - \mu\mathbf{1}\rVert^2= (\mathbf{d} - \mu\mathbf{1})^\top (\mathbf{d} - \mu\mathbf{1})
```
...
\Large
\begin{align}
\arg\min_{\mu} \boxed{\lVert\mathbf{d} - \mu\mathbf{1}\rVert^2  } =\arg\min_\mu \boxed{\sum_i (d_i -\mu)^2}
\end{align}
```
