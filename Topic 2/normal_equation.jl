...
In this lecture, we are going to derive the normal equation from a **geometric** perspective
...
``w_1`` = $(@bind v₁_ Slider(-2:0.1:3, default=.5, show_value=true)) 
...
> ```math
> \large
>  (\mathbf{y} - \mathbf{Xw})^\top \mathbf{X} = \mathbf{0} 
> ```
...
```math
\begin{align}
\Rightarrow & \mathbf{X}^\top (\mathbf{y} - \mathbf{Xw}) = \mathbf{0}^\top \tag{apply ⊤ on both sides}\\
...
Note that the *single projection matrix* is a specific case (where ``\mathbf{X}`` is single column vector ``\mathbf{x}``)
...
\
\
When ``\color{blue}\mathbf{y}``: the ``\textcolor{blue}{\rm blue}`` vector ``\in \{\mathbf{Xw}\}``
...
When ``\color{blue}\mathbf{y}``: the ``\textcolor{blue}{\rm blue}`` vector **_sticks out_**

or 

```math
\textcolor{blue}{\mathbf{y}} \notin \{\mathbf{Xw}, \mathbf{w}\in \mathbb{R}^m\}
```
* **NO solution**
...
\hat{\mathbf{w}}  = (\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{X}^\top\mathbf{y}
```
The projection (or the prediction) is
\
\
